...
abstract class NioSocketAcceptor extends SimpleChannelInboundHandler<SentBody>{
	...
	/**
	 *  三次握手，接受syn包创建连接时，会read数据
	 *  getOuterRequestHandler().process，调用BindHandler进行处理
	 */
	@Override
	...
	/**
	 *  四次挥手断开连接(主动断开或者异常关闭)，触发unregister和inactive事件
	 *  getOuterRequestHandler().process，这里调用CloseHandler进行处理
	 */
	@Override
